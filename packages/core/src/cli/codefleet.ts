#!/usr/bin/env node
/**
 * Thin shell/CI wrapper over CodeFleet — no interactive session, approvals,
...
import { CodeFleet } from '../orchestrator/orchestrator.js'
...
class CodeFleetValidationError extends Error {
...
function asTeamConfig(v: unknown, label: string): TeamConfig {
  if (!isObject(v)) throw new CodeFleetValidationError(`${label}: expected a JSON object`)
  const name = v['name']
  const agents = v['agents']
  if (typeof name !== 'string' || !name) throw new CodeFleetValidationError(`${label}.name: non-empty string required`)
  if (!Array.isArray(agents) || agents.length === 0) {
    throw new CodeFleetValidationError(`${label}.agents: non-empty array required`)
  }
  for (const a of agents) {
    if (!isObject(a)) throw new CodeFleetValidationError(`${label}.agents[]: each agent must be an object`)
    if (typeof a['name'] !== 'string' || !a['name']) throw new CodeFleetValidationError(`agent.name required`)
    if (typeof a['model'] !== 'string' || !a['model']) {
      throw new CodeFleetValidationError(`agent.model required for "${String(a['name'])}"`)
    }
  }
...
function asTaskSpecs(v: unknown, label: string): ReadonlyArray<{
  ...
  if (!Array.isArray(v)) throw new CodeFleetValidationError(`${label}: expected a JSON array`)
  const out: Array<{
    title: string
    description: string
    assignee?: string
  ...
  let i = 0
  for (const item of v) {
    if (!isObject(item)) throw new CodeFleetValidationError(`${label}[${i}]: object expected`)
    if (typeof item['title'] !== 'string' || typeof item['description'] !== 'string') {
      throw new CodeFleetValidationError(`${label}[${i}]: title and description strings required`)
    }
...
function help(): string {
  return [
    'codefleet CLI (codefleet)',
    '',
    'Usage:',
    '  codefleet run --goal <text> --team <team.json> [--orchestrator <orch.json>] [--coordinator <coord.json>]',
    '  codefleet task --file <tasks.json> [--team <team.json>]',
    '  codefleet provider [list | template <provider>]',
    '',
    'Flags:',
    '  --pretty              Pretty-print JSON to stdout',
    '  --include-messages    Include full LLM message arrays in run output (large)',
    '  --dashboard           Write team-run DAG HTML dashboard to codefleet-dashboards/',
    '',
      ...
      printJson(
        {
          error: {
            kind: 'usage',
            message: `usage: codefleet provider template <${PROVIDER_REFERENCE.map((p) => p.id).join('|')}>`,
          },
        },
        pretty,
      )
...
async function writeRunTeamDashboardFile(html: string): Promise<string> {
  const directory = join(process.cwd(), 'codefleet-dashboards')
...
async function main(): Promise<number> {
  ...
  try {
    if (cmd === 'run') {
      ...
      const orchestrator = new CodeFleet(mergeOrchestrator({}, ...orchParts))
      ...
      if (dashboard) {
        ...
        try {
          ...
          process.stderr.write(
            `codefleet: failed to write runTeam dashboard: ${err instanceof Error ? err.message : String(err)}\n`,
          )
        }
      }
    ...
    if (cmd === 'task') {
      ...
      if (!isObject(doc)) {
        throw new CodeFleetValidationError('tasks file root must be an object')
      }
      ...
      if (tasks.length === 0) {
        throw new CodeFleetValidationError('tasks array must not be empty')
      }
...
function classifyCliError(e: unknown, message: string): { kind: string; exit: number } {
  if (e instanceof CodeFleetValidationError) return { kind: 'validation', exit: EXIT.USAGE }
