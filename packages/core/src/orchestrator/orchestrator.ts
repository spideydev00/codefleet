/**
 * @fileoverview CodeFleet — the top-level multi-agent orchestration class.
 *
 * {@link CodeFleet} is the primary public API of the codefleet framework.
 ...
 *
 * ```ts
 * const orchestrator = new CodeFleet({ defaultModel: 'claude-opus-4-6' })
 *
...
/**
 * Top-level orchestrator for the codefleet framework.
 *
...
export class CodeFleet {
  ...
  createTeam(name: string, config: TeamConfig): Team {
    if (this.teams.has(name)) {
      throw new Error(
        `CodeFleet: a team named "${name}" already exists. ` +
   ...
   * {@link CodeFleet.runFromPlan} — a restored `runTeam` run returns the raw
