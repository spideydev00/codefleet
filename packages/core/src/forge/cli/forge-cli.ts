/**
 * @fileoverview Thin injectable command-line interface for Forge.
 */

import {
  runForge,
  type RunForgeOptions,
} from '../orchestrator/run-forge.js'

const USAGE = [
  'Usage: forge [options] <prompt>',
  '       forge [options] --prompt <prompt>',
  '',
  'Options:',
 ...
 */
export interface ForgeCliDependencies {
...
export async function runCli(
  argv: string[],
  dependencies: ForgeCliDependencies = {},
): Promise<number> {
  ...
  try {
    ...
    const result = await (dependencies.runForge ?? runForge)(parsed.options)
