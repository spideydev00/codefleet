/**
 * @fileoverview Schema and DAG validation for planned CodeFleet tasks.
 */
 ...
 *
 * @throws {CodeFleetValidationError} for schema, identity, reference, or cycle errors.
 */
export function parseTasksPlan(raw: unknown): TasksPlan {
  ...
  if (!parsed.success) {
    const issues = parsed.error.issues
      ...
      .join('; ')
    throw new CodeFleetValidationError(`Invalid TasksPlan: ${issues}`)
  }
  ...
  for (const task of plan.tasks) {
    if (ids.has(task.id)) {
      throw new CodeFleetValidationError(`Duplicate task id: ${task.id}`)
    }
    ids.add(task.id)
  }
  ...
  for (const task of plan.tasks) {
    for (const dependency of task.dependsOn) {
      if (dependency === task.id) {
        throw new CodeFleetValidationError(`Task "${task.id}" cannot depend on itself`)
      }
