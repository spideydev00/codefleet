/**
 * @fileoverview Schema and parser for executable CodeFleet task briefs.
 */
 ...
 *
 * @throws {CodeFleetValidationError} when the input violates the schema.
 */
export function parseTaskBrief(raw: unknown): TaskBrief {
  ...
  const issues = parsed.error.issues
    ...
    .join('; ')
  throw new CodeFleetValidationError(`Invalid TaskBrief: ${issues}`)
}
