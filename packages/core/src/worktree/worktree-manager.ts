/**
 * @fileoverview Git-worktree and directory isolation for CodeFleet tasks.
 */
...
export class WorktreeManager {
  ...
  constructor(options: WorktreeManagerOptions) {
    ...
    this.rootDir = resolve(
      options.rootDir ?? join(tmpdir(), 'codefleet', this.runId),
    )
  ...
  async create(taskId: string, fromRef?: string): Promise<Workspace> {
    return await this.enqueue(async () => {
      ...
      const branch = `codefleet/${this.runId}/${taskId}`
  ...
  private async withTemporaryIndex<T>(
    ...
    const tempRoot = await mkdtemp(join(tmpdir(), 'codefleet-index-'))
