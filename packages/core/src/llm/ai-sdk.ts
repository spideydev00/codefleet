...
// ---------------------------------------------------------------------------
// Message conversion — CodeFleet <-> AI SDK ModelMessage
// ---------------------------------------------------------------------------
...
function usageToCodeFleet(u: { inputTokens?: number | undefined; outputTokens?: number | undefined }): TokenUsage {
  return {
...
function buildLlmResponseFromGenerateText(result: {
  ...
  return {
    ...
    usage: usageToCodeFleet(result.usage),
  }
}
    ...
    try {
      ...
      for await (const part of result.fullStream) {
        if (part.type === 'text-delta') {
          ...
          usage = usageToCodeFleet(part.totalUsage)
        }
      }
      ...
      if (totalUsage !== undefined) {
        usage = usageToCodeFleet(totalUsage)
      }
