...
describe('CodeFleet', () => {
  ...
  describe('createTeam', () => {
    it('creates and registers a team', () => {
      ...
      const team = codefleet.createTeam('my-team', teamCfg())
      expect(team.name).toBe('test-team')
      expect(codefleet.getStatus().teams).toBe(1)
    })

    it('throws on duplicate team name', () => {
      const codefleet = new CodeFleet()
      codefleet.createTeam('my-team', teamCfg())
      expect(() => codefleet.createTeam('my-team', teamCfg())).toThrow('already exists')
    })
  })
  ...
  describe('shutdown', () => {
    it('clears teams and counters', async () => {
      const codefleet = new CodeFleet()
      codefleet.createTeam('t1', teamCfg())
      await codefleet.shutdown()
      expect(codefleet.getStatus().teams).toBe(0)
      expect(codefleet.getStatus().completedTasks).toBe(0)
    })
  })
  ...
  describe('getStatus', () => {
    it('reports initial state', () => {
      const codefleet = new CodeFleet()
      const status = codefleet.getStatus()
  ...
  describe('runAgent', () => {
    it('runs a single agent and returns result', async () => {
      ...
      const result = await codefleet.runAgent(
        agentConfig('solo'),
        'Say hello',
      )
    ...
    it('fires onProgress events', async () => {
      ...
      await codefleet.runAgent(agentConfig('solo'), 'test')
  ...
  describe('runTasks', () => {
    ...
    it('routes leaf worker tasks to an explicit cheaper model without mutating the team', async () => {
      ...
      const codefleet = new CodeFleet({ defaultModel: 'premium-model' })
      const team = codefleet.createTeam('t', teamCfg([
    ...
    it('routes critical dependent tasks to an explicit premium model', async () => {
      ...
      const codefleet = new CodeFleet({ defaultModel: 'default-model' })
      const team = codefleet.createTeam('t', teamCfg([
        { ...agentConfig('worker-a'), model: 'default-worker-model' },
      ]))
  ...
  describe('runTeam', () => {
    it('runs coordinator decomposition + execution + synthesis', async () => {
      ...
      const result = await codefleet.runTeam(team, 'First research AI safety best practices, then write a comprehensive implementation guide')
    ...
    it('falls back to one-task-per-agent when coordinator output is unparseable', async () => {
      ...
      const result = await codefleet.runTeam(team, 'First design the database schema, then implement the REST API endpoints')
    ...
    it('supports coordinator model override without affecting workers', async () => {
      ...
      const result = await codefleet.runTeam(team, 'First research the topic, then synthesize findings', {
    ...
    it('appends coordinator.instructions to the default system prompt', async () => {
      ...
      await codefleet.runTeam(team, 'First implement, then verify', {
        coordinator: {
    ...
    it('applies advanced coordinator options (maxTokens, temperature, tools, disallowedTools)', async () => {
      ...
      await codefleet.runTeam(team, 'First inspect project, then produce output', {
        coordinator: {
          maxTurns: 5,
          maxTokens: 1234,
          temperature: 0,
    ...
    it('omits team-context block by default (revealCoordinator unset)', async () => {
      ...
      await codefleet.runTeam(team, 'First research the topic, then synthesize findings')
    ...
    it('injects team-context block when revealCoordinator is true', async () => {
      ...
      await codefleet.runTeam(team, goal, { revealCoordinator: true })
    ...
    it('injects team-context block into delegated worker prompts when revealCoordinator is true', async () => {
      ...
      const codefleet = new CodeFleet({ defaultModel: 'mock-model', maxConcurrency: 3 })
      const team = codefleet.createTeam('t', teamCfg([
        {
      ...
      await codefleet.runTeam(team, goal, {
    ...
    it('does not run final synthesis when abortSignal is aborted after task execution', async () => {
      ...
      const result = await codefleet.runTeam(team, 'First do the work, then synthesize the result', {
        coordinator: { adapter: coordinatorAdapter },
    ...
    it('marks tasks with unknown coordinator dependencies as failed instead of dropping them', async () => {
      ...
      const result = await codefleet.runTeam(team, 'First resolve dependencies, then produce output', {
        coordinator: { adapter: coordinatorAdapter },
      })
    ...
    it('fails ambiguous title dependencies when coordinator emits duplicate task titles', async () => {
      ...
      const result = await codefleet.runTeam(team, 'First research in parallel, then synthesize', {
        coordinator: { adapter: coordinatorAdapter },
      })
    ...
    it('includes failed and skipped task sections in final synthesis prompt', async () => {
      ...
      await codefleet.runTeam(team, 'First run independent work, then review the remaining step', {
        coordinator: { adapter: coordinatorAdapter },
      })
  ...
  describe('onApproval gate', () => {
    it('skips remaining tasks when approval rejects', async () => {
      ...
      const team = codefleet.createTeam('t', teamCfg([agentConfig('worker')]))
  ...
  describe('onPlanReady gate', () => {
    ...
    it('emits plan_ready trace even when onPlanReady callback is not configured', async () => {
      ...
      const result = await codefleet.runTeam(team, complexGoal)
  ...
  describe('planOnly mode', () => {
    ...
    it('returns the decomposed plan without executing tasks', async () => {
      ...
      const result = await codefleet.runTeam(team, complexGoal, { planOnly: true })
    ...
    it('bypasses simple-goal short-circuit when planOnly is true', async () => {
      ...
      const result = await codefleet.runTeam(team, simpleGoal, { planOnly: true })
    ...
    it('emits balanced agent_start / agent_complete for the coordinator when planOnly is true', async () => {
      ...
      await codefleet.runTeam(team, complexGoal, { planOnly: true })
    ...
    it('creates a serializable artifact from planOnly output and replays without coordinator', async () => {
      ...
      const planOnlyResult = await codefleet.runTeam(team, complexGoal, { planOnly: true })
      const plan = codefleet.createPlanArtifact(planOnlyResult)
      ...
      const replay = await codefleet.runFromPlan(team, plan)
