/**
 * @fileoverview Tests for the injectable Forge command-line interface.
 */

import { describe, expect, it } from 'vitest'
import { runCli } from '../../src/forge/cli/forge-cli.js'
import type { RunForgeOptions } from '../../src/forge/orchestrator/run-forge.js'
import type { ForgeReport } from '../../src/forge/report-types.js'

function report(status: ForgeReport['status']): ForgeReport {
  return {
    runId: 'run-1',
...
describe('runCli', () => {
  it.each(['--help', '-h'])('prints usage to stdout for %s', async flag => {
    ...
    expect(output[0]).toContain('Usage: forge')
  ...
  it('parses supported flags and prints JSON output', async () => {
    let received: RunForgeOptions | undefined
    const output: string[] = []
  ...
  it('prints usage and returns 64 for invalid arguments', async () => {
    ...
    expect(called).toBe(false)
    expect(errors[0]).toContain('Usage: forge')
  })
