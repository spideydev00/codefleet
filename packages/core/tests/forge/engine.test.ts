...
import { runPlan } from '../../src/forge/engine/engine.js'
...
async function expectForgeClean(runId: string): Promise<void> {
  const worktrees = await runGit(repoRoot, ['worktree', 'list', '--porcelain'])
  expect(worktrees.stdout.match(/^worktree /gm) ?? []).toHaveLength(1)
  const branches = await runGit(repoRoot, ['branch', '--list', `forge/${runId}/*`])
  expect(branches.stdout.trim()).toBe('')
}

beforeEach(async () => {
  testRoot = await mkdtemp(join(tmpdir(), 'forge-engine-test-'))
...
describe('runPlan', () => {
  ...
  it('does not retry failures and transitively skips dependents', async () => {
    ...
    expect(bRuns).toBe(0)
    await expectForgeClean('failure')
  })
  ...
  it('uses the injected resolver for conflicting successful tasks', async () => {
    ...
    await expectForgeClean('conflict')
  })
  ...
  it('preserves all run worktrees and branches when requested', async () => {
    ...
    const branches = await runGit(repoRoot, ['branch', '--list', `forge/${runId}/*`])
    expect(branches.stdout).toContain(`forge/${runId}/a`)
    expect(branches.stdout).toContain(`forge/${runId}/integration`)
  })
})
