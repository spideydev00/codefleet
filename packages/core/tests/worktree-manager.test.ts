/**
 * @fileoverview Integration tests for isolated CodeFleet task workspaces.
 */
...
import { resolveCommit, runGit } from '../src/worktree/git.js'
...
beforeEach(async () => {
  testRoot = await mkdtemp(join(tmpdir(), 'codefleet-worktree-test-'))
...
describe('WorktreeManager', () => {
  it('creates a branch-backed workspace outside the repository', async () => {
    ...
    expect(workspace.branch).toBe(`codefleet/${manager.runId}/task-a`)
