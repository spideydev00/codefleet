...
**Shell & CI.** Use the JSON-first `codefleet` binary. See [docs/cli.md](https://github.com/spideydev00/codefleet/blob/main/docs/cli.md).
...
Using `codefleet` in production or a side project? [Open a discussion](https://github.com/spideydev00/codefleet/discussions) and we will list it here.
...
Limited-time provider offers for `codefleet` users. Listings are not paid endorsements.
...
For products and platforms with a deep `codefleet` integration. See the [Featured partner program](https://github.com/spideydev00/codefleet/blob/main/docs/featured-partner.md) for terms and how to apply.
...
- [`integrations/with-vercel-ai-sdk`](examples/integrations/with-vercel-ai-sdk/): Next.js app combining CodeFleet `runTeam()` with AI SDK `useChat` streaming.
...
- **Translations.** Help translate this README into other languages. [Open a PR](https://github.com/spideydev00/codefleet/pulls).
