# Command-line interface (`codefleet`)

The package ships a small binary **`codefleet`** that exposes the same primitives as the TypeScript API: `runTeam`, `runTasks`, plus a static provider reference. It is meant for **shell scripts and CI** (JSON on stdout, stable exit codes).
...
Runs **`CodeFleet.runTeam(team, goal)`**: coordinator decomposition, task queue, optional synthesis.

When invoked with `--dashboard`, the **`codefleet` CLI** writes a static post-execution DAG dashboard HTML to `codefleet-dashboards/runTeam-<timestamp>.html` under the current working directory (the library does not write files itself; if you want this outside the CLI, call `renderTeamRunDashboard(result)` in application code — see `packages/core/src/dashboard/render-team-run-dashboard.ts`).
...
| `--orchestrator` | No | Path to JSON merged into `new CodeFleet(...)` after any orchestrator fragment from the team file. |
| `--coordinator` | No | Path to JSON passed as `runTeam(..., { coordinator })` (`CoordinatorConfig`). |
| `--dashboard` | No | Write a post-execution DAG dashboard HTML to `codefleet-dashboards/runTeam-<timestamp>.html`. |

Global flags: [`--pretty`](#output-flags), [`--include-messages`](#output-flags).

### `codefleet task`

Runs **`CodeFleet.runTasks(team, tasks)`** with a fixed task list (no coordinator decomposition).
...
- **`codefleet provider`** or **`codefleet provider list`** — Prints JSON: built-in provider ids, API key environment variable names, whether `baseURL` is supported, and short notes (e.g. OpenAI-compatible servers, Copilot in CI).
- **`codefleet provider template <provider>`** — Prints a JSON object with example `orchestrator` and `agent` fields plus placeholder `env` entries. `<provider>` is one of: `anthropic`, `azure-openai`, `openai`, `gemini`, `grok`, `minimax`, `mimo`, `deepseek`, `doubao`, `qiniu`, `copilot`, `bedrock`.
...
### `codefleet`, `codefleet help`, `codefleet -h`, `codefleet --help`
...
Used with **`codefleet run --team`** (and optionally **`codefleet task --team`**).
...
```bash
npx codefleet run --goal "Summarize README" --team team.json > result.json
code=$?
case $code in
  0) echo "OK" ;;
