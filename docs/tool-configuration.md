...
//    from the repo root.
const defaultOrchestrator = new CodeFleet()
...
const wideOrchestrator = new CodeFleet({
  defaultCwd: process.cwd(),
})

// 3. Disable the sandbox entirely (relative and absolute paths anywhere).
const unrestrictedOrchestrator = new CodeFleet({
  defaultCwd: null,
})
```
...
```typescript
import { defineTool } from '@codefleet/core'
import { z } from 'zod'
...
`codefleet` can connect to stdio MCP servers and expose their tools directly to agents.

```typescript
import { connectMCPTools } from '@codefleet/core/mcp'
